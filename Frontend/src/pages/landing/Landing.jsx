...
// Mock recent reviews (keep for now until Reviews API is built)
const recentReviews = [
  {
    id: 1,
    user: 'Ahmed K.',
...
const Landing = () => {
  ...
  useEffect(() => {
    const fetchTrending = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/restaurants/trending');
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      // You can implement this route or handle search globally
      // For now we'll just redirect to restaurants page
        ...
        <div className="container">
          <div className="row align-items-center min-vh-75 py-5">
            <div className="col-lg-6 mb-5 mb-lg-0">
              ...
              {/* Search Bar */}
              <form onSubmit={handleSearch} className="input-group input-group-lg mb-3 shadow-sm rounded-pill overflow-hidden">
                ...
                />
                <button type="submit" className="btn btn-orange px-4" id="hero-search-btn">
                  Search
                </button>
              </form>
                  ...
                  <Link to={`/restaurants/${restaurant._id}`} className="text-decoration-none">
                    <div className="card border-0 shadow-sm card-hover h-100">
                      <div className="position-relative">
                        ...
                        {restaurant.badge && <span className="badge bg-dark position-absolute top-0 start-0 m-2">{restaurant.badge}</span>}
